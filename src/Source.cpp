...
/*
#define GET_VAR_NAME(name) getVarName(#name)		//Recives varibale and
returns it's name

string getVarName(const char *name) {				//function used
to return a Dtring with the variable's name, complements the #define above
        string s(name);
        return s;
}
*/
...
public:
  ...
  Animal(string name, bool fourLegs, bool domestic, bool carnivor, bool mamifer,
         bool topFoodChain, bool fly, bool feline) {
    this->name = name;
  ...
  // gets a String and recognizes wich variable is it being asked for based on
  // the string literally makes all the other gets useless
  ...
  int HashCode() {
    return (fourLegs * pow(2, 1) + domestic * pow(2, 2) + carnivor * pow(2, 3) +
            mamifer * pow(2, 4) + topFoodChain * pow(2, 5) + fly * pow(2, 6) +
            feline * pow(2, 7));
  }
};

// this classe saves a question and the related variable's name
// edit 1: this clas will only save the question, the variable's name will be
// used as the Animal's variable name edit 2: press F for the ideia that I had
// on edit 1 xD
class Question {
...
int main() {
  ...
  Animal dolphin =
      Animal("Dolphin", false, false, true, true, true, false, false);
  Animal codfish =
      Animal("Codfish", false, false, true, false, false, false, false);
  Animal dog = Animal("Dog", true, true, true, true, true, false, false);
  Animal cat = Animal("Cat", true, true, true, true, true, false, true);
  Animal elephant =
      Animal("Elephant", true, false, false, true, true, false, false);
  ...
  Animal chicken =
      Animal("Chicken", false, true, false, false, false, false, false);
  ...
  Question topFoodChain("Is the animal on the top of the food chain?",
                        "topFoodChain");
  ...
  while (1) {
    ...
    if (opt == 1) {
      ...
      cout << "\nChoose an animal an then answer the following questions, I "
              "will try to guess wich one did you choose";
      cout << endl
           << "Please anwer "
              "y"
              " to "
      ...
      do {
        ...
        do {
          ...
          if (flag == true)
            cout << endl
                 << "Please anwer "
                    ...
                    ""; // shows a help
          cout << endl
               << questIndex->getQuestion() << endl; // shows the question
          ...
          flag = true; // puts the falg = true to the case where the user gives
                       // wrong input
        ...
        for (int i = 0; i < cpyAnimalList.size(); i++) {
          ...
          if (cpyAnimalList[i]->getByString(questIndex->getCaracteristic()) !=
              answerAsBool) {
          ...
          if (cpyAnimalList.size() == 0) {
            cout << "I can not find an animal with such caracteristics."
                 << endl;
            return 0;
          }
        }
      ...
      cout << endl
           << "The animal must be:\t" << (*cpyAnimalList.begin())->getName()
           << endl;
...
clearscr();
      ...
      cout << "name:\t" << name << endl;
      cout << fourLegs.getQuestion() << (fourLegCar ? "\ttrue" : "\tfalse")
           << endl;
      cout << domestic.getQuestion() << (domesticatedCar ? "\ttrue" : "\tfalse")
           << endl;
      cout << carnivor.getQuestion() << (carnivourCar ? "\ttrue" : "\tfalse")
           << endl;
      cout << mamifer.getQuestion() << (mamiferCar ? "\ttrue" : "\tfalse")
           << endl;
      cout << topFoodChain.getQuestion()
           << (topFoodChainCar ? "\ttrue" : "\tfalse") << endl;
      cout << fly.getQuestion() << (canFlyCar ? "\ttrue" : "\tfalse") << endl;
      cout << feline.getQuestion() << (felineCar ? "\ttrue" : "\tfalse")
           << endl;
      ...
      if (add == 'y') {
        ...
        Animal newAnimal =
            Animal(name, fourLegCar, domesticatedCar, carnivourCar, mamiferCar,
                   topFoodChainCar, canFlyCar, felineCar);
        ...
        if (checksIfCanBeAdded(animalList, newAnimal)) {
          ...
          cout << endl
               << name
               << "was not added to the list, there is a animal with the same "
                  "caracteristics there!";
        }
...
/*
        checks if there is another animal with the same caracteristics already
   in the List return false if there is an animal with the same caracteristics
